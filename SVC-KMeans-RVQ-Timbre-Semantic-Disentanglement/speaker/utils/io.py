...
def load_fsspec(
    path: str,
    map_location: Union[str, Callable, torch.device,
                        Dict[Union[str, torch.device], Union[str, torch.device]]] = None,
    **kwargs,
) -> Any:
...
def load_checkpoint(model, checkpoint_path, use_cuda=False, eval=False):  # pylint: disable=redefined-builtin
    try:
        ...
        state = load_fsspec(checkpoint_path, map_location=torch.device(
            "cpu"), pickle_module=pickle_tts)
    ...
    **kwargs,
    ...
    if current_loss < best_loss:
        ...
        if not keep_all_best or (current_step < keep_after):
            model_names = fs.glob(os.path.join(
                out_path, "best_model*.pth.tar"))
