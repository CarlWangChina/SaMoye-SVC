...
class Coqpit(Serializable, MutableMapping):
    ...
    def load_json(self, file_name: str) -> None:
        ...
        self = self.deserialize(
            dump_dict)  # pylint: disable=self-cls-assignment
        self.check_values()
    ...
    def parse_args(
        ...
        for k, v in args_dict.items():
            ...
            try:
                ...
                raise Exception(
                    f" [!] '{k}' not exist to override from argparse.") from e
...
def check_argument(
    ...
    if is_path:
        assert os.path.exists(
            c[name]), f' [!] path for {name} ("{c[name]}") does not exist.'
    ...
    if name in c.keys():
        ...
        if enum_list is not None:
            assert c[name].lower(
            ) in enum_list, f" [!] {name} is not a valid value"
