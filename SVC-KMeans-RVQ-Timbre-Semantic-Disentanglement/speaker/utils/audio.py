...
class AudioProcessor(object):
    ...
    def __init__(
        ...
        **_,
        ...
        if stats_path and signal_norm:
            mel_mean, mel_std, linear_mean, linear_std, _ = self.load_stats(
                stats_path)
    ...
    def _stft_parameters(
        ...
        ), " [!] frame_shift_ms should divide frame_length_ms"
    ...
    def denormalize(self, S: np.ndarray) -> np.ndarray:
        ...
        if self.signal_norm:
            ...
            if self.symmetric_norm:
                ...
                S_denorm = ((S_denorm + self.max_norm) * -self.min_level_db /
    ...
    def load_stats(self, stats_path: str) -> Tuple[np.array, np.array, np.array, np.array, Dict]:
        ...
        skip_parameters = ["griffin_lim_iters", "stats_path",
                           "do_trim_silence", "ref_level_db", "power"]
    ...
    def compute_stft_paddings(self, x, pad_sides=1):
        ...
        pad = (x.shape[0] // self.hop_length + 1) * \
            self.hop_length - x.shape[0]
        if pad_sides == 1:
            return 0, pad
