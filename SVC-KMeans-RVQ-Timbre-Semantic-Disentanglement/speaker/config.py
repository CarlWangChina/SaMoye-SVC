...
class SpeakerEncoderConfig(BaseTrainingConfig):
    ...
    storage: Dict = field(
        default_factory=lambda: {
            # the probability with which we'll sample from the DataSet in-memory storage
    ...
    # training params
    # end training when number of training steps reaches this value.
    max_train_step: int = 1000000
    loss: str = "angleproto"
    grad_clip: float = 3.0
    lr: float = 0.0001
    lr_decay: bool = False
