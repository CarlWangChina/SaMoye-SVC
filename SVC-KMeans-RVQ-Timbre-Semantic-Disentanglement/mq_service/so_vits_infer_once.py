...
synthesizer_infer_model = SynthesizerInfer(
...
def svc_infer_pipeline(
    ...
    """
    model = load_svc_model(model_path, synthesizer_infer_model)
...
def svc_infer_pipeline_panxin_choice_best(
    wave: str,
    spk_id: str,
    model_path: str,
    save_name: str = "svc_out.wav",
    ...
    """Test for auto f0 shift and choice the best
    ...
    logger.info(f"pitch shift: {shifts}")

    batch_wavNames = []
    for shift in shifts:
        tmppit = deepcopy(pit)
        if shift != 0:
            tmppit = np.array(tmppit)
            source = tmppit[tmppit > 0]
            source_ave = source.mean()
            source_min = source.min()
            source_max = source.max()
            logger.info(
            ...
            tmppit = tmppit * (2 ** (shift / 12))
        tmppit = torch.FloatTensor(tmppit)
        retrieval = DummyRetrieval()
        out_audio = svc_infer(model, retrieval, spk,
                              tmppit, ppg, vec, hp, device)

        new_wav, _ = post_process(ref_wave_path=wave, svc_wave=out_audio)

        final_save_name = save_name + f"{shift}.wav"
        logger.info(f"save {final_save_name} with shift {shift}")
        write(final_save_name, hp.data.sampling_rate, new_wav)
        batch_wavNames.append(final_save_name)

    # 选择最佳的shift

    choices = {}
    for filename in batch_wavNames:
        tmppit = pitch_infer(filename)
        # min
        # score = sum(np.abs((tmppit - tmppit.min()) -
        #                    (pit - pit.min())))
        # mean
        score = sum(np.abs((tmppit - tmppit.mean()) -
                           (pit - pit.mean())))
        choices[filename] = score.numpy()

    sorted_choices = sorted(choices.items(), key=lambda x: x[1])
    logger.info(f"{sorted_choices}")

    for ele in sorted_choices:
        final_save_name = os.path.splitext(
            ele[0])[0] + f"_{int(ele[1])}" + ".wav"
        os.rename(ele[0], final_save_name)
        logger.info(f"{ele[0]} -> {final_save_name}")

    return out_audio, final_save_name


def svc_infer_pipeline_panxin_choice_stage2(
    wave: str,
    spk_id: str,
    model_path: str,
    save_name: str = "svc_out.wav",
    ...
    # Spk pitch
    spkPit = np.load(f"data_svc/pitch/{spk_id}/{spk_id}.npy")
    ...
    shift = 0
    if spkPit[2] < np.mean(pit.numpy()) * f0scale:
        pit = np.array(pit)
        source = pit[pit > 0]
        source_ave = source.mean()
        source_min = source.min()
...
def svc_infer_pipeline_panxin_choice_f0shift(
    wave: str,
    spk_id: str,
    model_path: str,
    save_name: str = "svc_out.wav",
    ...
    """Test for auto f0 shift and auto f0 shift choice by mean comparation.
    ...
    model = load_svc_model(model_path, synthesizer_infer_model)
    vad_model = init_jit_model("vad/assets/silero_vad.jit", device)
    vad_model.eval()
    ...
    pit = pitch_infer(wave, f0_predictor="rmvpe")
    # logger.info(f"pitch infer start ...")
    # pit = pitch_infer(wave, "rmvpe")
    ...
    for shift in shifts:
        ...
        if shift != 0:
            ...
            tmppit = tmppit * (2 ** (shift / 12))
        np.save(save_name + f"{shift}.npy", tmppit)
        tmppit = torch.FloatTensor(tmppit)
        retrieval = DummyRetrieval()
        out_audio = svc_infer(model, retrieval, spk,
                              tmppit, ppg, vec, hp, device)
        new_wav, _ = post_process(
            vad_model, ref_wave_path=wave, svc_wave=out_audio)
...
def post_process(vad_model, ref_wave_path: str, svc_wave: str):
    """_summary_
    ...
    tag_wave = get_speech_timestamps(
        tmp_wave, vad_model, threshold=0.139, sampling_rate=16000
    )
