...
def save_audio(path: str,
               tensor: torch.Tensor,
               sampling_rate: int = 16000):
    torchaudio.save(path, tensor.unsqueeze(
        0), sampling_rate, bits_per_sample=16)
...
def get_speech_timestamps(audio: torch.Tensor,
    ...
    if return_seconds:
        for speech_dict in speeches:
            speech_dict['start'] = round(
                speech_dict['start'] / sampling_rate, 1)
...
def get_number_ts(wav: torch.Tensor,
    ...
    perframe_preds = torch.argmax(torch.softmax(
        perframe_logits, dim=1), dim=1).squeeze()   # (1, num_frames_strided)
    extended_preds = []
    ...
    for i, pred in enumerate(extended_preds):
        if pred == 1:
            if not triggered:
                cur_timing['start'] = int(
                    (i * hop_length) / (sample_rate / 1000))
                triggered = True
        elif pred == 0:
            if triggered:
                cur_timing['end'] = int(
...
def get_language_and_group(wav: torch.Tensor,
    ...
    for i in range(top_n):
        ...
        outs_group.append(
            (lang_group_dict[str(srtd_group[i].item())], prob_group))
