...
def main(midi_path, output_dir, lyric_data=None):
    ...
    spks = ["male1", "male2", "female1"]
    run_diffsinger_command_test(ds_input_path, ds_output_path)
    for spk in spks:
        vocal_spk_path = vocal_path / spk
        vocal_spk_path.mkdir(exist_ok=True)
        mp3_path = vocal_spk_path / f"{file_name}.mp3"
        if mp3_path.exists():
            continue
        run_diffsinger_command_test(
            ds_output_path=ds_output_path, vocal_path=vocal_spk_path, spk=spk
        )


if __name__ == "__main__":
    # python script/task4_25_midi_to_json_to_vocal/midi_to_json_to_vocal_5-16_test_time_range.py
    midi_dir = root_path / "data/human_midi/test_time_range"
    output_dir = root_path / "data/human_midi_output/test_time_range"
