...
@click.option('--device', type=str, required=False, default='None', help='Device to run the inference')
def acoustic(
        proj: str,
        exp: str,
        ckpt: int,
    ...
    if device == 'cpu':
        infer_ins = DiffSingerAcousticInfer(device = device, load_vocoder=not mel, ckpt_steps=ckpt)
    else:
    ...
    # # print(f"Max GPU memory reserved: {max_memory_reserved / 1024 / 1024:.2f} MB")
    
    # import csv
    # # Prepare the data
    # data = [proj, f'{process_time:.2f}', f'{infer_ins.total_time:.2f}', f'{time_tatio:.2f}' ,f'{max_memory / 1024 / 1024:.2f}', f'{max_memory_reserved / 1024 / 1024:.2f}']
    # # Write the data to a CSV file
    # with open('acoustic_output.csv', 'a', newline='') as file:
