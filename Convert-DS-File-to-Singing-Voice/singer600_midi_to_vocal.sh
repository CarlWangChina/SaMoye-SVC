...
# 默认参数值
TEST_NAME="singer600_2"
TEST_MIDI_FILE="singer600_2_test1"
SPK="[\"cpop_female\"]"
BATCH_SIZE="100"
CUDA="7"
MODIFIED="2"
...
# python midi_lyric_align.py --test_name $TEST_NAME --test_midi_file $TEST_NAME --modified $MODIFIED
# echo ""

# 发送第二个curl请求：根据ds获取新的midi模版，该midi模版位于data/midi_for_lyric/$TEST_NAME/目录下
# python ds_to_midi.py --test_name $TEST_NAME --test_midi_file $TEST_NAME
# echo ""
# exit
...
if [ "$(ls -A data/midi/$TEST_MIDI_FILE)" ]; then
  ...
  python change_ds_midi.py --test_name $TEST_NAME --test_midi_file $TEST_MIDI_FILE --overwrite 0
fi
...
for song_id in $song_ids; do
  ...
  echo "/export/data/home/john/MuerSinger2/data/ds/$TEST_MIDI_FILE/"$song_id".ds"
  # 发送第三个curl请求：根据ds转换的midi生成新的midi，该midi位于data/midi/$TEST_MIDI_FILE/目录下
  sudo /home/xary/anaconda3/envs/melody/bin/python /export/data/home/xary/projects/melody_generation/Pitch-Regeneration/infer.py \
    --path "/home/john/MuerSinger2/data/midi_for_lyric/$TEST_NAME/"$song_id".mid" \
...
# 发送第六个curl请求：混音
python remix.py --test_name $TEST_NAME --test_midi_file $TEST_MIDI_FILE
# echo ""