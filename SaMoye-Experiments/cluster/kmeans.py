...
def _kpp(data: torch.Tensor, k: int, sample_size: int = -1):
    ...
    if batch_size > sample_size:
        data = data[torch.randint(
            0, batch_size, [sample_size], device=data.device)]
    ...
    for i in range(k):
        if i == 0:
            ...
            D2 = torch.cdist(init[:i, :][None, :], data[None, :], p=2)[
                0].amin(dim=0)
...
class KMeansGPU:
    ...
    def __init__(self, n_clusters, max_iter=200, tol=1e-4, verbose=0, mode="euclidean", device=torch.device("cuda:0")):
        ...
        self.minibatch = int(33e6 / self.n_clusters *
                             info.free / 1024 / 1024 / 1024)
        print("free_mem/GB:", info.free / 1024 /
              1024 / 1024, "minibatch:", self.minibatch)
    ...
    #                        torch.double], "input must be floating point"
    ...
    #             x = X[torch.randint(
    #                 0, batch_size, [int(self.minibatch * 10 / offset)])].to(self.device)
    #         else:
    #             x = X.to(self.device)
    #         # print(x.device)
    #         self.centroids = _kpp(x, self.n_clusters, min(
    #             int(self.minibatch / 12 / offset), batch_size))
    #         del x
    #         torch.cuda.empty_cache()
    #         # self.centroids = self.centroids.to(self.device)
    #         num_points_in_clusters = torch.ones(
    #             self.n_clusters, device=self.device, dtype=X.dtype)  # 全1
    #         closest = None  # [3098036]#int64
    #         if (self.minibatch >= batch_size // 2 and self.minibatch < batch_size):
    #             X = X[torch.randint(0, batch_size, [self.minibatch])].to(
    #                 self.device)
    ...
    #             closest = self.max_sim(a=x, b=self.centroids)[1].to(
    #                 torch.int16)  # [3098036]#int64#0~999
    #             matched_clusters, counts = closest.unique(
    #                 return_counts=True)  # int64#1k
    #             expanded_closest = closest[None].expand(
    #                 self.n_clusters, -1)  # [1000, 3098036]#int16#0~999
    #             mask = (expanded_closest == torch.arange(self.n_clusters, device=self.device)[
    #                     :, None]).to(X.dtype)  # ==后者是int64*1000
        ...
        with torch.no_grad():
            ...
            for i in range(self.max_iter):
                ...
                distances = torch.norm(
                    x - self.centroids[closest.long()], dim=1)
                inertia = min(inertia, torch.sum(distances ** 2))

            if self.verbose >= 1:
                print(
                    f'used {i+1} iterations ({round(time()-start_time, 4)}s) to cluster {batch_size} items into {self.n_clusters} clusters')
        return closest, inertia
