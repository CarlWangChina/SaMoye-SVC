...
class WN(torch.nn.Module):
    ...
    def forward(self, x, x_mask, style_embed, **kwargs):
        output = torch.zeros_like(x)
        n_channels_tensor = torch.IntTensor([self.hidden_channels])

        style_embed = self.cond_layer(style_embed)

        for i in range(self.n_layers):
            x_in = self.in_layers[i](x)

            cond_offset = i * 2 * self.hidden_channels
            g_l = style_embed[:, cond_offset : cond_offset + 2 * self.hidden_channels, :]
...
class ResidualCouplingLayer(nn.Module):
    ...
    def forward(self, x, x_mask, style_embed, reverse=False):
        ...
        h = self.enc(h, x_mask, style_embed)
...
class MelStyleEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.out = LinearNorm(self.hidden_dim, self.out_dim)
